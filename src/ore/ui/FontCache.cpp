...
std::filesystem::path ore::ui::FontCache::findTTFInDirectory(const std::filesystem::path &directoryToSearch, const std::string& fontFileName) {
    try {
...
std::filesystem::path ore::ui::FontCache::findTTFFile(const std::string &fontFileName) {
...
void ore::ui::FontCache::loadFont(nk_context *context, ore::ui::Font face, unsigned int size) {
    struct nk_font_atlas *atlas = nullptr;
    struct nk_font_config config = nk_font_config(0);
    struct nk_font *font = nullptr;
    std::filesystem::path ttfFile;

    if(face == Font::defaultFont) {
        ttfFile = fontFileLocations.at(ore::ui::Font::arial);
    } else {
        ttfFile = fontFileLocations.at(face);
    }
...
void ore::ui::FontCache::setFont(nk_context *context, ore::ui::Font face, unsigned int size) {
...
void ore::ui::FontCache::initialize() {
    for(const std::pair<const ore::ui::Font, std::vector<std::string>> &fontFaceAlternatives : TTFFilenames) {
